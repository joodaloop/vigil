...
export function App() {
  ...
  const [timeseries, setTimeseries] = createSignal<Row[]>([]);

  const graphTitle = () => {
    const i = interval();
    return i === "today" || i === "yesterday" || i === "1d"
      ? "Visits per hour"
      : "Visits per day";
  };
  ...
  async function load() {
    ...
    setStatus("Loading…");
    try {
      const [c, p, r, co, b, d, ts] = await fetchAll(siteId(), interval());
      setCounts(c);
      setPaths(p);
      setReferrers(r);
      setCountries(co);
  ...
  const bounceRate = () => {
    ...
    return visitors > 0 ? Math.round((bounces / visitors) * 100) + "%" : "—";
  };
  ...
  return (
    <>
      <h1>Vigil</h1>
      <div class="controls">
        <select value={siteId()} onChange={(e) => updateSiteId(e.currentTarget.value)}>
          <Show when={siteId() && !sites().includes(siteId())}>
            <option value={siteId()}>{siteId()}</option>
          </Show>
          <For each={sites()}>{(s) => <option value={s || "@unknown"}>{s || "(unknown)"}</option>}</For>
        </select>
        <select value={interval()} onChange={(e) => updateInterval(e.currentTarget.value)}>
      ...
      <Graph title={graphTitle()} rows={timeseries()} />
...
function TopList(props: { title: string; rows: Row[]; label: (r: Row) => string; value: (r: Row) => string }) {
  return (
